...
The mock configuration lives next to the CLI and produces lightweight TSV/PDF placeholders so the command can be executed end to end while still signalling where real outputs would be generated.

> **Tip:** YAML configs require `PyYAML`. If that dependency is unavailable, provide the same settings via JSON (for example, `mock_config.json`).

### Runtime dependencies

The automated runner depends on the scientific Python stack that underpins Scanpy. Make sure the execution environment provides the following packages (or their equivalents) before launching the workflow (or enable mock mode as described above):
